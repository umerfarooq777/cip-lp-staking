...
const headers = ["CIP Amount", "Date", "Time"];

const itemsPerPage = 6;
const WithdrawalHistory = ({ data, loading }: any) => {
  ...
  return (
    <>
      <h2 className="text-2xl font-medium py-8">Withdrawal History</h2>
          ...
          <div className="overflow-auto lg:overflow-visible ">
            <table className="table border-separate space-y-6 text-sm w-full">
              ...
              <tbody className="font-semibold">
                {paginatedData?.map((rowData: any, index: number) => (
            ...
            {/* {!paginatedData.length ? <TextRow>{currentWalletAddress ? loading ? "Loading..." : "No Records to Show." : "Please connect wallet"}</TextRow> : <></>} */}
            <Pagination
...
const TableRow = ({ data, index }: TableRowProps) => {
  ...
  return (
    <tr className="bg-neutral-900 text-center">
      ...
      <td className="p-3 text-start">{formattedTime}</td>
    </tr>
  );
};
